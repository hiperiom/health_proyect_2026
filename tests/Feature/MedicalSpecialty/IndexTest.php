...
it('loads the medical-specialties index page for the superusuario', function () {
    MedicalSpecialty::factory()->count(3)->create();

    actingAs($this->user)
        ->get(route('medical-specialties.index'))
        ->assertOk()
        ->assertInertia(
            fn ($page) => $page
                ->component('medical-specialties/Index')
                ->has('items.data', 3)
        );
});
...
it('redirects guests to the login page', function () {
    get(route('medical-specialties.index'))->assertRedirect('/login');
});
